...
class CmdNode(Node):
    ...
    def start(self, msg):
        ...
        if (dt > 0.05):
            ...
            self.twist.angular.z = map_value(msg.axes[2] - msg.axes[5], -2.0, 2.0,
    ...
    def set_speed_and_publish(self):
        data = [
            ...
            bytes(struct.pack("f", float(self.twist.x))),
            bytes(struct.pack("f", float(self.twist.y))),
            bytes(struct.pack("f", float(self.twist.z)))]
        data = b''.join(data)
